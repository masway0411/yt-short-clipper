...
// API Keys Save
apiView.fields.saveBtn.addEventListener('click', async () => {
  const geminiKey = apiView.fields.geminiKey.value.trim();
  const youtubeKey = apiView.fields.youtubeKey.value.trim();

  apiView.fields.status.textContent = 'Saving...';

  try {
    // Save to localStorage for now
    localStorage.setItem('gemini_api_key', geminiKey);
    localStorage.setItem('youtube_api_key', youtubeKey);
    apiView.fields.status.textContent = 'API Keys saved successfully!';
    setTimeout(() => {
      apiView.fields.status.textContent = '';
    }, 3000);
  } catch (e) {
    apiView.fields.status.textContent = 'Error saving API keys';
  }
});
...
async function init() {
  ...
  try {
    ...
    apiView.fields.geminiKey.value = geminiKey;
    apiView.fields.youtubeKey.value = youtubeKey;
  } catch {}
...
apiView.fields.geminiEye.addEventListener('click', () => toggleEye(apiView.fields.geminiKey, apiView.fields.geminiEye));
apiView.fields.youtubeEye.addEventListener('click', () => toggleEye(apiView.fields.youtubeKey, apiView.fields.youtubeEye));
